...
/* Rule for inferencing FB having neither initial token nor repetition -- РАБОТАЕТ !!! */
function_block(Id,Color,false,false,NRep):-
transition(Id,FromState,ToState,Color,NRep),
not(initial_state(FromState)), 
FromState\==ToState.

/* Rule for inferencing FB having no initial token but having repetition -- FALSE ?????? */
function_block(Id,Color,false,true,NRep):-
transition(Id,State,State,Color,NRep),
not(initial_state(State)).

/* Rule for inferencing FB having initial token but not having repetition -- РАБОТАЕТ !!! */
function_block(Id,Color,true,false,NRep):-
transition(Id,FromState,ToState,Color,NRep),
initial_state(FromState), 
FromState\==ToState.

/* Rule for inferencing FB having initial token and repetition -- НЕ достаточно инстанитиирована */
...
/* ============================== */
/* Automaton A3 (an example for testing the program) */
/* ============================== */

/* PROLOG facts for representation of automaton A3 */
