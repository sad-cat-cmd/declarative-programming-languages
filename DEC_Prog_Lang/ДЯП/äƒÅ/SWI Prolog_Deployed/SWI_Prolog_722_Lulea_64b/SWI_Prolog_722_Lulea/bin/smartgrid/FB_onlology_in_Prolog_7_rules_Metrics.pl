...
writeln("----------------------------------------------------"),
dynamic('FunctionBlock'/1), 'FunctionBlock'(Fb), hasName(Fb, NameFb), has_FB_type(Fb, TypeFb),
writeln("FB name: "), write(NameFb), writeln("FB type: "), write(TypeFb), write_FBRes(Fb),
writeln("EventInputs of FB: "), write_FBEventInputs(Fb).

write_FBRes(Fb):-  is_mapped_to(Fb, Res), has_name(Res, NameRes), writeln("Resource: "), write(NameRes).
write_FBRes(_):-  writeln("Resource missing !!!"),

write_FBEventInputs (Fb):- dynamic('EventInput'/1), 'EventInput'(Ei), is_part_of_FB(Ei, Fb), 
hasName(Ei, NameEi), string_concat(NameEi, "; ", X), writeln(X), fail.
write_FBEventInputs (_).
