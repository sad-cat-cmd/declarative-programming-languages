...
/* Enumeration of all event paths between two FB */

find_path(SourceFB, TargetFB, Path):-
find_path(SourceFB, TargetFB, [SourceFB], Path).
find_path(Node, Node, _, [Node]).                    
find_path(Start, Finish, Visited, [Start | Path]) :-
     edge(Start, X),
     not(member(X, Visited)),
     find_path(X, Finish, [X | Visited], Path).
