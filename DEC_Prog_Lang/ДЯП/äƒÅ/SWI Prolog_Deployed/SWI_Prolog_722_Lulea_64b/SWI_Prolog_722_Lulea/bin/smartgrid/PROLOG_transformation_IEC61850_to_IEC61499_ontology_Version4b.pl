...
/* OUTPUT:  IEC 61499 ontology based on PROLOG. The newly generated facts of this ontology are output to separate files (it is for simplicity and convenience). 
  ...
  Moreover, it is printed on the user screen the following important information to visualize and validate the results of transformation:
  -- statiscics (metrics) about IEC 61850 and  IEC 61499 ontologies
  -- information about all FBs including their name, type, inputs, outputs, etc. */
...
write_FB_statistics,
writeln("==="),
writeln(">>> Please press any key and dot to continue the output (for example, a.)"), 
read(X),
write_all_FB_info.
...
createLink(has_var_type, Ov, LncType),
createLink(has_var_type, Iv, LncType),

string_concat(LncName, "_Ev", EO_name),
createLink(hasName, Eo, EO_name), 
string_concat(LncName, "_Ev", EI_name),
