...
/* Query6: “All devices in the System have distinct names” */
/* ---------------- */

all_Devs_in_System_have_distint_names:-
findall(DevName, has_Device_Name2(Sys, DevId, DevName), L),
unique(L).

has_Device_Name2(Sys, DevId, DevName):-
has_Device(Sys, DevId),
has_Device_Name(DevId, DevName).

/* Query7a: “Every FB from an application or a subapplication must be mapped to a resource” */
/* ---------------- */

every_FB_is_mapped:- 
forall(fb_in_SubAppNetwork(FB), fb_is_mapped(FB)).

fb_is_mapped(FB):-  has_FB_Name(FB, FBName), atom_string(FBName,FBNameStr), 'Mapping'(M), has_Mapping_From(M, From), split_string(From, ".", "", L), last(L, FBNameStr), !.

fb_in_SubAppNetwork(FB):- 'SubAppNetwork'(SAN), has_FB(SAN, FB),!.


/* Query7b: “Every FB from an application or a subapplication is mapped to a single-type FB on a resource” */
/* ---------------- */

every_fb_is_mapped_to_single_type_fb:- 
forall(fb_in_SubAppNetwork(FB), fb_is_mapped_to_single_type_fb(FB)).

fb_is_mapped_to_single_type_fb(FB):-  
has_FB_Name(FB, FBName1), atom_string(FBName1,FBNameStr1), has_FB_Type(FB, Type), 'Mapping'(M), has_Mapping_From(M, From), split_string(From, ".", "", L1), last(L1, FBNameStr1), has_Mapping_To(M, To), split_string(To, ".", "", L2), last(L2, RStr), atom_string(R, RStr),  'Resource'(R), has_FBNetwork(R, Net), has_FB(Net, FB2), has_FB_Type(FB2, Type), !.

/* Query8: “Every FB [everywhere in whole System] has at least one input event” */
/* ---------------- */

every_FB_has_at_least_one_input_event_in_System:-
forall('FBNetwork'(Net), every_FB_has_at_least_one_input_event_in_FBNetwork(Net)).

every_FB_has_at_least_one_input_event_in_FBNetwork(Net):-
forall(has_FB(Net, FB), fb_has_at_least_one_input_event(Net, FB)).

fb_has_at_least_one_input_event(Net, FB):-
has_FB_Name(FB, FBName), atom_string(FBName,FBNameStr),
has_EventConnections(Net, Conns), has_Connections(Conns, Conn), has_Connection_Source(Conn, Src), split_string(Src, ".", "", [FBNameStr |_]), !.

/* Query9: “Find (enumerate) FBs [everywhere in whole System] which have no input event” */
/* ---------------- */
fbs_with_no_input_events(WrongFBs):-
findall(FB, fb_having_no_input_events(FB), WrongFBs).

/* This predicate can be used in enumeration of FBs */
fb_having_no_input_events(FB):- 
'FB'(FB), fb_having_no_input_events2(FB).

/* This predicate can be used in the determining FB only because it uses the cut operation*/
fb_having_no_input_events2(FB):- 
has_FB(Net, FB), has_FB_Name(FB, FBName), atom_string(FBName,FBNameStr),
has_EventConnections(Net, Conns), has_Connections(Conns, Conn), has_Connection_Source(Conn, Src), split_string(Src, ".", "", [FBNameStr |_]), !, false.
fb_having_no_input_events2(X).

/* Query10a: “There are event inputs specified in a list in a given FB” */

there_are_event_connections([], FB):- !.
there_are_event_connections(EventList, FB):-
EventList=[EI|Tail], 
'FB'(FB),  
has_FB_Name(FB, FBName), 
atom_string(FBName,FBNameStr), 
has_FB(Net, FB), 
has_EventConnections(Net, Conns), 
has_Connections(Conns, Conn), 
has_Connection_Destination(Conn, Dst), 
atom_string(Dst, DstStr),
split_string(DstStr, ".", "", L), 
last(L, EI), 
there_are_event_connections(Tail, FB).




/* Query10b: “Find (enumerate) FB types where there exist the event inputs (specified in a list)” */

find_FB_types_where_exist_needed_event_inputs(EventList, FBTNames):- 
findall(FBTName, 
fb_type_where_exist_needed_event_inputs(EventList, FBTName), FBTNames).

fb_type_where_exist_needed_event_inputs([], _):- !.
fb_type_where_exist_needed_event_inputs(EventList, FBTName):-
has_FBType_Name(FBT, FBTName),
'FBType'(FBT),
has_InterfaceList(FBT, IL),
has_EventInputs(IL, EIs),
has_Event(EIs, Ev),
has_Event_Name(Ev, EvName),
atom_string(EvName, EvNameStr),
EventList=[EvNameStr | Tail],
fb_type_where_exist_needed_event_inputs(Tail, FBTName).
